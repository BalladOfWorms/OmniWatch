...
local function apply_properties(target_id, resource, action_id,
    ...
    local epoch = os.time()   -- wall-clock epoch at creation, paired with clock
    resonating[target_id] = {
        ...
        -- Stable epoch timestamps computed ONCE here (clock and epoch are
        -- sampled together, so the offset is consistent). Resends reuse
        -- these verbatim instead of recomputing — prevents the ±1s jitter
        -- that made the Python countdown bar stutter every heartbeat.
        delay_ts    = epoch + delay,
        window_ts   = epoch + delay + 8 - (step or 1),
        step        = step,
...
-- Is this actor the player, or a party/alliance member? Only their
-- skillchains are ones the player can contribute to — without this gate,
-- a nearby OTHER group's trust/player weaponskilling a mob created a
-- resonating entry and made the SC panel falsely suggest contribution.
-- Cached for 1s so we don't rebuild the id set on every action.
local _sc_ally_ids = nil
local _sc_ally_ts  = 0
local function _sc_actor_is_ally(actor_id)
    if not actor_id or actor_id == 0 then return false end
    local p = windower.ffxi.get_player()
    if p and actor_id == p.id then return true end
    -- Own pet/trust directly summoned.
    if p and p.pet and p.pet.id and actor_id == p.pet.id then return true end

    local now = os.clock()
    if not _sc_ally_ids or (now - _sc_ally_ts) >= 1.0 then
        local set = {}
            ...
            for slot, m in pairs(party) do
                if type(slot) == 'string' and type(m) == 'table'
                   and m.mob and m.mob.id then
                    set[m.mob.id] = true
                    -- Include each member's pet/trust by pet_index → id.
                    if m.mob.pet_index and m.mob.pet_index ~= 0 then
                        local petmob = windower.ffxi.get_mob_by_index(
                            m.mob.pet_index)
                        if petmob and petmob.id then set[petmob.id] = true end
                    end
                end
            end
        end
    ...
    return _sc_ally_ids[actor_id] == true
end
...
function M.handle_action(act)
    ...
    -- Only track skillchains from the player or party/alliance (incl.
    -- their trusts/pets). Skip other groups' fights entirely — otherwise
    -- a nearby skillchain falsely drives our suggestion panel.
    if not _sc_actor_is_ally(actor) then return end
...
function M.tick(now_clock)
    ...
    for tid, r in pairs(resonating) do
        if r.created < stale_cutoff then
            ...
            -- the chain is over. For a closed chain, keep it ~10s longer
            -- so the magic-burst window still shows; otherwise drop it as
            -- soon as the window ends (+1s grace). Without this the entry
            -- lingered until the 30s GC, so the panel kept showing (and
            -- blinking) a dead chain.
            local win_clock = r.window_end
            if win_clock then
                local tail = r.closed and 11 or 1
                if now_clock > (win_clock + tail) then
                    resonating[tid] = nil
                end
            end
        end
    ...
    if key ~= last_emit_key or (now_t - last_emit_time) > 0.5 then
        -- Use the stable timestamps captured at creation (no per-send
        -- recompute → no ±1s jitter → smooth Python countdown bar).
        local delay_ts = reson.delay_ts
            or math.floor(os.time() + (reson.delay_until - now_t))
        local win_ts   = reson.window_ts
            or math.floor(os.time() + (reson.window_end  - now_t))
