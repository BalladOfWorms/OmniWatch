...
While sim mode is open, the **EQUIPMENT panel is taken over** to show the gear you've picked rather than your live set — its title reads **EQUIPMENT — SIM** so the state is obvious. Slots you haven't touched keep showing your real gear (sim seeds them from what you're wearing on open), and "(empty)" picks show the slot unequipped. Closing sim mode restores the live view. **Hovering an item** in the sim window — a slot's chosen item, or any row in an open gear dropdown — shows the same rich item card the equipment panel uses (description, item level, jobs, augments); equipped pieces show their full card, and augments reflect the specific copy you're hovering.
