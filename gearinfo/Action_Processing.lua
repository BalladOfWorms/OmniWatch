					...
					for index, m_table in pairs(member_table) do
						if member_table[index].id == action.actor_id then
							...
							if rollNum == 12 and #rollMembers > 0 then
								...
								if settings.player.show_COR_messages and chars then
					...
					if spells_to_watch:contains(spell.en) then
						for index, m_table in pairs(member_table) do
							if member_table[index].id == action.actor_id then
								...
								-- ── OmniWatch patch ─────────────────────────
								-- 0x063's matched_exactly branch (in
								-- Packet_parsing.lua line 268) gates buff
								-- full_name/Caster assignment on:
								--   Last_Spell ~='' AND value~=0 AND effect~=''
								-- Vanilla code only sets Last_Spell here, so
								-- the gate fails for songs and the buff
								-- enters buff_details with full_name=nil,
								-- which makes check_buffs (Buff_Processing)
								-- skip it. Set sentinel non-empty values so
								-- the gate passes; check_buffs's bard branch
								-- doesn't read these fields.
								member_table[index].effect = 'song'
								member_table[index].value  = 1
								-- ── end patch ───────────────────────────────
							end
