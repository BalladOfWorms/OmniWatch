...
A travel-and-tidy release plus a batch of chat upgrades ported over from the OmniChat side project. Two new headline features: a **Warp button** — a floating, pulsing one-click travel menu for Home Points and Survival Guides, fired through the superwarp addon — and **inventory right-click actions** that drop an item or hand it to the Treasury addon's auto-drop list. On the chat side: **focus words** (define words or phrases in the Filters GUI and the matched word itself pulse-highlights in amber, any channel, any tab, with inactive tabs blinking their label until visited), **Filters GUI saves applying live** (no more reloading OmniWatch after editing routing), and **auto-translate sending** from the composer with a one-click `{ }` wrap button.
...
- **Auto-translate sending from the composer** — write `{Yes, please.}`-style phrases in the composer (or click the new **{ }** button: with text typed it wraps the whole message, click again to unwrap; empty, it inserts `{}` with the cursor parked inside). On send, the lua side swaps each `{…}` matching an auto-translate phrase (case-insensitive, English or Japanese names) for the real 6-byte in-game token via `windower.chat.input()`, so JP players see it in Japanese exactly as if Tab-selected natively. Unmatched braces send literally. Incoming auto-translate already renders as `{phrase}`, so anything you see in the panel can be copied straight back into the composer. Hotbar `CMD\|input …` commands get the same treatment.
...
- **Lua AT encoder stored on `_G`** — the encoder helpers (`_ow_encode_at`, `_ow_dispatch_console`, the lazy name→id index) are globals rather than file-locals because OmniWatch.lua's main chunk is at Lua 5.1's 200-local limit (the same constraint that puts `_ow_slips` on `_G`). The encoding is the exact byte-for-byte inverse of the existing incoming decode (`FD 02 02 id_hi id_lo FD`), keyed off the same `res.auto_translates` table.
