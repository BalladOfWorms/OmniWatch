...
A correctness and polish pass, plus first-class support for running OmniWatch on multiple FFXI clients at once. The big-ticket items: a **multibox display lock** that ends the panel "jitter" when two logged-in characters both feed the overlay (with the chat panel pinnable to a chosen main character); a long-standing **party panel bug fixed** so real (non-trust) party members finally display, with their jobs now read live from packets; Master-Level subjob evasion now tiers correctly; the Unity Ranking gear-stat scaling formula was reverse-engineered from in-game data and fixed (it had been over-counting on every non-first-place Unity); equipment tooltips became rich FFXIAH-style stat blocks; and the linkshell message-of-the-day display was overhauled so both LS1 and LS2 show one clean, correctly-named, correctly-colored line.

### Added

- **Multibox display lock** — running OmniWatch on more than one FFXI client at once is now supported. Previously both clients sent their live data to the same overlay ports and the panels would flip ("jitter") back and forth between the two characters several times a second. Each client now tags its live-display data with the sending character's name, and the overlay locks the display to one character — whoever you pick in the header character selector — and drops the other character's packets. Switching the selected character switches the whole display (party, equipment, stats, target, zone, recasts, buffs, DPS, inventory) to that character, and the panels clear-and-refresh on switch so you don't briefly see the previous character's data. Single-client use is unaffected.
- **Chat pinned to a main character** — in a multibox setup, the chat panel can be locked to one "main" character independently of which character's panels you're viewing, so chat stays put while you flip the rest of the display between characters. Set it in the **Header Configure** modal via the **Chat main character** picker (just above the Server row); the list is populated from the characters OmniWatch currently sees logged in. Defaults to following the live character until you pick one.
- **Party member jobs from packets** — the party panel now reads same-zone members' main/sub jobs and levels directly from the game's party-update (`0x0DD`) and char-update (`0x0DF`) packets, parsed via the Windower `packets` library (the same approach xivparty uses). `get_party()` doesn't expose other members' jobs, so they previously showed blank for everyone but you; now they fill in for any member in your zone. (Out-of-zone members still can't report jobs — the game doesn't send that data.) New `//ow partydebug` toggles a trace of what's parsed.
...
- **Chat composer: removed the "reply" channel** — the composer channel picker dropped the `reply` (`/r`) entry; it was redundant with `tell` and rarely the intended channel. The remaining channels (say / tell / party / shout / yell / ls1 / ls2 / unity / assist) are unchanged and the picker still cycles by `<` / `>`.
...
- **Party panel now shows real (non-trust) members** — the party encoder required a member to have a rendered world entity (`member.mob`) before displaying them, but the game only populates that for entities in your immediate render range. Trusts are always beside you so they always qualified — which masked the bug — but real party members who were out of range, zoning, or in another area had a valid name with no `mob` and were silently dropped. The panel now shows any member with a name, and all entity-derived fields (buffs, id, pet) degrade gracefully when the member isn't in range.
...
- **Per-character stream tagging + gate** — every live-display payload is prefixed with the sending character's name as `@name@` on the Lua side (`_OW_MB_TAG`); the Python overlay strips and checks it in one chokepoint (`_mb_gate`), accepting only the locked character's packets. Untagged packets pass through unchanged, so single-client use and any non-tagged stream are unaffected. Chat datagrams are tagged in `chat/drain.lua` and gated with a separate "pin to main" target.
- **Party jobs via the packets library** — the `0x0DD`/`0x0DF` handler uses `packets.parse('incoming', data)` (the required `packets` module, as the base already does for `0x061`/`0x028`) rather than hand-rolled byte offsets, and stores results in a module-global table (`_OW_PARTY_JOBS`) so the deep packet handler and the deep party encoder share one source regardless of Lua scope depth.
- **Dynamic enum options** — the Header settings "Chat main character" picker fills its options at draw time from the live character list; `set_setting`/`load_settings`/coercion were taught to validate `chat_main_char` against the dynamic list (and to accept any character name) so a chosen main isn't snapped back to the default by the static schema.
