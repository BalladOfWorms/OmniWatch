...
### Added
- **GearSwap dual-wield integration (replaces HasteInfo)**: OmniWatch now pushes your computed "DW Needed" value to GearSwap so engaged sets can swap automatically as haste buffs change — the same role the HasteInfo addon fills. The value is derived from your real gear, traits, JP gifts, and active haste buffs, and is sent as the self-command `gs c owdw <N>`. It only fires when the integer value changes, and never while sim mode is active so what-if sims don't trigger real set swaps. See the README's "Driving GearSwap dual-wield sets from OmniWatch" section for the `process_owdw` handler to add to a job/globals file. If migrating from HasteInfo, point your self-command handler at `process_owdw` and stop loading HasteInfo so the two don't both push and fight over the value.
- **Treasure Hunter readout on the target card**: when on THF main or /THF, the target card shows the current TH tier ("TH - N") on a mob, parsed from the Added-Effect proc messages in the action stream and tracked per-mob with a 10-minute TTL. Auto-hidden when there's no proc. (This had existed earlier and was lost in the PartyWatch → OmniWatch rename; rebuilt.)
...
### Fixed
- **Dual-wield path-augment accuracy applied to both hands** (`Gear_Processing.lua` + `OmniWatch.lua`): for REMA / Dynamis-Divergence (DREMA) weapons, a weapon's path-augment Accuracy/Attack is now applied per the in-game `/checkparam` truth. The augment acc/att is held out of GearInfo's shared stat bucket (which feeds both hands) and re-applied via OmniWatch's per-slot path-augment pass, so the displayed Primary/Auxiliary accuracy matches the game instead of leaking or double-counting. The weapon's BASE accuracy/attack still applies to both hands as normal. Strictly scoped to weapons listed in `DREMA_augments.lua` — non-DREMA weapons are untouched.
- **Warp Ring (and other enchantment items) showed "uses ?" in the overlay** (`chat/battle_events.lua`): enchantment-item use fires a 0x028 action whose packet-level param doesn't resolve to an item, so the name fell back to "?". The resolvable item id is in the per-target `action.param` (e.g. Warp Ring → 28540), which the cat-9 handler now reads first (falling back to the packet-level param). Restores "uses Warp Ring" and fixes the whole family of cat-9 enchantment/consumable items that share this packet shape. (Regression from the PartyWatch → OmniWatch rename.)
- **Jute Boots +1 (and other Unity items) showed phantom augment stats when unaugmented** (`Gear_Processing.lua`): a Unity-item rule applied the static augment block (e.g. +15 DEX/AGI) to every equipped Unity item by id regardless of its actual augment state, so an unaugmented Unity piece showed augment stats it didn't have. The augment block is now gated on the item actually carrying a real decoded augment; the unconditional Unity rank bonus is unaffected.
- **DREMA augment loader filename casing** (`OmniWatch.lua`): the loader looked for `DREMA_Augments.lua` (capital A) but the shipped file is `DREMA_augments.lua` (lowercase). Corrected the load path and warning text. Worked on case-insensitive Windows but was fragile.
...
- **Your own outgoing auto-translate phrases rendered as garbage in party/say/linkshell** (`OmniWatch.lua` + `chat/emit.lua`): your own outgoing chat reaches the panel via Windower's incoming-text echo, which delivers auto-translate phrases pre-mangled — the phrase ID's high byte is dropped, so e.g. `{test}` (id 7801) rendered as a stray "y". Sent tells (resolved from the 0x0B6 outgoing packet) and yells (round-tripped as inbound 0x017) were unaffected; party/say/linkshell/shout/emote had no intact source. OmniWatch now hooks the `outgoing text` event, which carries the typed command with the auto-translate sequence fully intact, resolves the phrase from those bytes, and substitutes the resolved text into the echo in place of the mangled fragment. Your own `{Auto-translate}` phrases now display correctly across those channels.
- **Stray leading "{" on some system messages** (`chat/emit.lua`): certain system-message modes (e.g. "There are no party members.") arrive with a literal `{` prefix byte that the native client hides during rendering but the addon received verbatim. The leading brace is now stripped on those specific system modes only, so a legitimate `{auto-translate}` phrase in real chat is never affected.
